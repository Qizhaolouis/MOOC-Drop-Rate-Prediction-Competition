---
title: "Feature Engineering with MOOC Dataset"
author: "Qi Zhao"
date: "`r Sys.Date()`"
output:
...
```
Except start month, all the columns we chose have a pretty strong linear correlation with the dropout probability. Notice that Pearson Correlation only indicates the linear relation.

Next, we want to look at the difference of the study time of students. We visualize the data using ggplot2.
```{r}
library(ggplot2)
ggplot(MOOC_train) +
  geom_histogram(aes(x = effective_time,
                     color = as.factor(dropout_prob)),
                 bins = 100) +
  facet_grid(.~ dropout_prob) +
  scale_x_log10()
```

As we can see, the study time of the students who didn't drop the class is obviously left skewed even after the transformation, which means usually they take more time to study a course.

Now let's look at the scatter plot which shows the relation of the time of the whole proccess and the time that the students have activities. The color denotes whether a student dropped a course.
```{r}
ggplot(MOOC_train) +
  geom_point(aes(x = proccess_period,
                 y = effective_time,
                 color = as.factor(dropout_prob)),
             size = 1,
             alpha = 0.6) 
...
Obviously, not only the students who passed the course has longer proccess time, but they have spent more time comparing to those who have the same learning proccess but has dropped the course.

# Prediction

Now let's use a very popular machine learning method called XGBoost to make submission. This is just for demonstrate so we use a small iterate times and a high learning rate to get an inaccurate model.
```{r  hide=TRUE}
## XGBoost
library(xgboost)
...
bst <- xgboost(data = train, label = label, max_depth = 3,
               eta = 1, nthread = 3, nrounds = 10, 
               objective = "binary:logistic")

```
Then let's make a submission function which makes it easy for submission.
```{r}
...
```
Great! Here's our final submission file.
```{r}
pred <- predict(bst, test)
submission(enrollment_id,pred,"XGBoost")
```
You can submit it through kaggle API or manually submit it on the site I have provided in the begining. I got a 0.33 log-loss using this simple one-layer model(without fine-tuning) and it is pretty good.
